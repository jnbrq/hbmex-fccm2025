...
def plotOne(
    ...
    seriesList: Tuple[str] = ("ID_ZERO_RAMA", "ID_MASK_INDEX", "ID_SHIFT_MASK_ADDR")
) -> List[plt.Artist]:
...
def plotFigurePresentation0() -> None:
    from matplotlib import pyplot as plt
    fig, axs = create_figure(width=9.2 * 0.7, height=5.2 * 0.7, top_extra=0.0, ncols=2)

    seriesList = ("ID_ZERO_RAMA", "ID_MASK_INDEX")
    ...
    handles = plotOne(axs[0], lambda dp: dp.len == 0, "Single-beat", annoList, seriesList)
    ...
    plotOne(axs[1], lambda dp: dp.len == 1, "2-beat", annoList, seriesList)

    axs[1].legend(handles=handles, loc="upper right", ncol=1)

    fig.supxlabel("Number of Pseudo Channels", weight="bold")
    fig.supylabel("Cycles/Beat", weight="bold")

    fig.savefig("presentation_figure0.pdf")

def plotFigurePresentation1() -> None:
    from matplotlib import pyplot as plt
    fig, axs = create_figure(width=5.4 * 0.7, height=5.2 * 0.7, top_extra=0.0, ncols=1)

    seriesList = ("ID_ZERO_RAMA", "ID_MASK_INDEX", "ID_SHIFT_MASK_ADDR")
    ...
    handles = plotOne(axs, lambda dp: dp.len == 0, "", annoList, seriesList)

    axs.legend(handles=handles, loc="upper right", ncol=1)

    fig.supxlabel("Number of Pseudo Channels", weight="bold")
    fig.supylabel("Cycles/Beat", weight="bold")

    fig.savefig("presentation_figure1.pdf")

plotFigurePresentation0()
plotFigurePresentation1()
