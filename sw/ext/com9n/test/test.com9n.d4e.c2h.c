...
int main()
{
    ...
    struct com9n_d4e_config cfg = {
        .addr_data = CMDQ_ADDR_GP_C2H_DATA,
        .addr_control = CMDQ_ADDR_GP_C2H_CONTROL,
        .capacity = 16,
        .size_msg = sizeof(uint32_t),
        .interrupt = 3
    };

    com9n_d4e_create(&cmdq, &cfg, &xil_device.device, COM9N_ROLE_RECV, u_mem_malloc_allocator);

    com9n_d4e_rrecv(&cmdq);

    U_VECTOR_ITERATE(uint32_t, elem, cmdq.messages)
    {
        printf("%llx\n", (unsigned long long)*elem);
    }

    com9n_d4e_rdone(&cmdq);
